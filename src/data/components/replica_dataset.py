...
class ReplicaDataset(Dataset):
    ...
    def __len__(self) -> int:
        return len(self.scan_names) * 50 if self.train else len(self.val_pairs)
        ...
        try:
            if self.train:
                ...
                while flag:
                    ...
                    candidates = items[idx1 + 10 : idx1 + 61]
                    candidates_stay = []
                    for idx2, candidate in enumerate(candidates):
                        if (
                            iou[context_id1, candidate] > 0.4
            ...
            context_views_images = [
                np.array(
                    Image.open(osp.join(scan_path, "color", f"{context_view}.jpg"))
                )
                for context_view in context_views_id
            ]
            target_views_images = [
                np.array(Image.open(osp.join(scan_path, "color", f"{target_view}.jpg")))
